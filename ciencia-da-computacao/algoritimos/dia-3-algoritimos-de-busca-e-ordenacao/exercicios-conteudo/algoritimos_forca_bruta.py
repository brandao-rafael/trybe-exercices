    ...
    for index in range(n - 1):  # Precisamos ordenar N-1 elementos
        ...
        for search_index in range(index + 1, n):  # Início da busca pelo menor
            ...
            if numbers[search_index] < numbers[min_element_index]:
                min_element_index = search_index  # Atualiza o índice atual do
                # menor elemento
...
def selection_sort_pythonic(numbers):
    n = len(numbers)

    for index in range(n - 1):  # Início da iteração para ordenar os N-1
        # elementos
...
def insertion_sort(numbers):
    ...
    for index in range(1, n):  # Começaremos a ordenar pelo segundo elemento
        ...
        # comparação
        while new_position >= 0 and numbers[new_position] > key:  # Enquanto a
            # chave for menor, remaneja o elemento para frente
