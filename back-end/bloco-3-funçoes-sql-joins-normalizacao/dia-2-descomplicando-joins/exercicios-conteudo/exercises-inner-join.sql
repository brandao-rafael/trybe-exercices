...
SELECT c.first_name, COUNT(a.address) AS `number_of_address`
FROM sakila.customer AS c
INNER JOIN sakila.address AS a
ON c.address_id = a.address_id
WHERE c.active
...
SELECT s.first_name, s.last_name, ROUND(AVG(p.amount), 2) AS 'average_value'
FROM sakila.staff AS s
