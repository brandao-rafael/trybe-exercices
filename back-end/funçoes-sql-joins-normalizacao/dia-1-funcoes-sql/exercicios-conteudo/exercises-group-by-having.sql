...
SELECT store_id, IF(`active` = 1, 'Ativo', 'Inativo') AS `status`, COUNT(*) AS `Quantidade de clientes`
FROM sakila.customer
...
SELECT rating, AVG(rental_duration) AS `average_rental`
FROM sakila.film
GROUP BY rating
...
SELECT district, COUNT(*) AS `residents`
FROM sakila.address
GROUP BY district
ORDER BY `residents`;
...
FROM sakila.film
GROUP by rating
HAVING `Total replacement cost` > 3950.50
